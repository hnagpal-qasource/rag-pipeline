...
if not os.getenv("GROQ_API_KEY"):
    st.warning("GROQ_API_KEY is not set. Add it to .env before asking questions.")
if sys.version_info >= (3, 14):
    st.error("Unsupported Python version detected. Use Python 3.11, 3.12, or 3.13.")
...
if question:
    if not st.session_state.get("chain_ready", False):
        st.error("Assistant is not ready yet. Fix backend initialization first.")
        st.stop()
    ...
    with st.chat_message("assistant"):
        try:
            ...
            st.error(f"Backend initialization failed: {exc}")
            response = "I couldn't answer because the backend failed to initialize."
